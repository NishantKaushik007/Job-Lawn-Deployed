...
interface DropdownProps {
  ...
  jobType: { company: string; value: string; code: string }[]; // Added jobType dropdown
...
const DropdownFilter: React.FC<DropdownProps> = ({
  ...
  industryExp,
  category,
  jobType, // Added jobType dropdown
  selectedCompany,
  currentParams,
}) => {
  ...
  const transformOptions = (data: { company: string; value: string; code: string }[]) => {
    return data
      .filter((item) => !selectedCompany || item.company === selectedCompany) // Filter by selected company if provided
