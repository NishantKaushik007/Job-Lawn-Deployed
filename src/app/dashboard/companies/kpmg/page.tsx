...
const fetchJobs = async (
  ...
  try {
    const queryParams = [
      filters.location && `selectedLocationsFacet=${filters.location[0]}`,
      filters.jobCategory && `selectedOrganizationsFacet=${filters.jobCategory[0]}`,
...
const fetchJobDetails = async (jobId: string): Promise<JobDetails | null> => {
  try {
    const url = `https://ejgk.fa.em2.oraclecloud.com/hcmRestApi/resources/latest/recruitingCEJobRequisitionDetails?expand=all&onlyData=true&finder=ById;Id=%22${jobId}%22`;
    const response = await fetch(url);
...
export default async function JPMorganChase({ searchParams }: PageProps) {
  ...
  const { location: selectedLocation, jobCategory: selectedJobCategory, postingDate: selectedPostingDate, page, keyword } = sp;

  const selectedCompany = 'KPMG';
