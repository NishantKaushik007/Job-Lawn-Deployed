...
async function fetchJobs(
  ...
  const tokenURL = "https://salesforce.wd12.myworkdayjobs.com/External_Career_Site";
  ...
  if (facets.country && facets.country.length > 0) {
    appliedFacets["CF_-_REC_-_LRV_-_Job_Posting_Anchor_-_Country_from_Job_Posting_Location_Extended"] = facets.country;
  }  
  ...
  const jobsEndpoint = "https://salesforce.wd12.myworkdayjobs.com/wday/cxs/salesforce/External_Career_Site/jobs";
  ...
  const jobs: Job[] = data.jobPostings.map((job: any) => {
    return {
      ...
      jobLink: `https://salesforce.wd12.myworkdayjobs.com/en-US/External_Career_Site${job.externalPath}`,
      descriptionLink: `https://salesforce.wd12.myworkdayjobs.com/wday/cxs/salesforce/External_Career_Site${job.externalPath}`,
      description: ""
    };
  });
...
const Page = async ({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) => {
  ...
  const selectedCompany = "Salesforce";
