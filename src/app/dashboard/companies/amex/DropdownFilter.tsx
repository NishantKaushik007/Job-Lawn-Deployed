...
interface DropdownProps {
  ...
  industryExp: { company: string; value: string; code: string }[];
  category: { company: string; value: string; code: string }[];
  discipline: { company: string; value: string; code: string }[];
  experienceLevel: { company: string; value: string; code: string }[];
  skills: { company: string; value: string; code: string }[];
...
const DropdownFilter: React.FC<DropdownProps> = ({
  ...
  return (
    <div className="flex flex-col mb-6 space-y-4 sm:flex-row sm:space-x-4 sm:space-y-0 w-full">
      ...
      />
      <Select
        value={getSelectedValue('industryExp', industryExp)}
        onChange={handleDropdownChange('industryExp')}
        options={transformOptions(industryExp)}
        className="w-full"
      ...
      />
      <Select
        value={getSelectedValue('discipline', discipline)}
        onChange={handleDropdownChange('discipline')}
        options={transformOptions(discipline)}
        className="w-full"
      ...
      <Select
        ...
        className="w-auto min-w-[15rem] max-w-full"
        placeholder="Select Experience Level"
        isClearable
        isSearchable
      />
      <Select
        value={getSelectedValue('skills', skills)}
        onChange={handleDropdownChange('skills')}
        options={transformOptions(skills)}
        className="w-full"
