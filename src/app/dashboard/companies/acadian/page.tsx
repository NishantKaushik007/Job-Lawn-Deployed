  ...
  if (!cache.data || currentTime - (cache.timestamp || 0) >= CACHE_EXPIRY_TIME) {
    ...
    try {
      const res = await fetch(
        "https://boards-api.greenhouse.io/v1/boards/acadianassetmanagementllc/jobs?content=true"
      );
      ...
      const offices = Array.from(
        new Set(jobList.flatMap((job) => job.offices.map((office) => office.location)))
      );
